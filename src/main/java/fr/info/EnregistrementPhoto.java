...
@WebServlet("/EnregistrementPhoto")
@MultipartConfig
public class EnregistrementPhoto extends HttpServlet {
	private static final long serialVersionUID = 1L;

	private EntityManagerFactory emf = Persistence.createEntityManagerFactory("DevTech");

	public EnregistrementPhoto() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		PhotoYao nouvellePhoto = new PhotoYao();
		nouvellePhoto.setNom(nom);
		nouvellePhoto.setDonnees(donnees);

		// Enregistrer la nouvelle photo dans la base de données
		em.persist(nouvellePhoto);
		tx.commit();
		em.close();
